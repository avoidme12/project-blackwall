...
run_shadow_web_meta() {
    ...
    if [ "$port" == "21" ]; then
        ...
        if [ $? -eq 0 ]; then
            echo "INFO:21|ACCESS FOUND: FTP ANONYMOUS ACCESS GRANTED!" >> "$log_file"
            if [ -n "$ftp_check" ]; then
                echo "INFO:21|FTP Directory Structure Extracted:" >> "$log_file"
    ...
    if [ "$port" == "443" ]; then
        ...
        local ssl_domains=$(openssl s_client -connect "${target}:443" -servername "${target}" 2>/dev/null </dev/null \
                ...
                if [ -n "$ssl_domains" ]; then
                    echo "INFO:443|ACCESS FOUND: SSL Domains in SAN TLS Cert listed!" >> "$log_file"
                    echo "$ssl_domains" | while read -r domain; do
                        echo "INFO:443|  -> Subject Alternative Name found: ${domain}" >> "$log_file"
            ...
            if [[ "$port" == "80" || "$port" == "443" ]]; then
                ...
                if [[ -n "$robots_content" && "$robots_content" == *"Disallow"* ]]; then
                    echo "INFO:${port}|ACCESS FOUND: robots.txt file discovered on target host!" >> "$log_file"
                ...
                if [ -n "$comments" ]; then
                    echo "INFO:${port}|ACCESS FOUND: Extracted HTML developer comments!" >> "$log_file"
